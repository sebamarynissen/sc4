...
describe('#cSC4City', function() {
	...
	it('parses a god mode tile', function() {
		...
		let city = entry.read();
		expect(city.date).to.eql(new Date('2000-01-01T12:00:00Z'));
	...
	it('parses a medium tile', function() {
		...
		let entry = dbpf.find({ type: FileType.cSC4City })!;
		let city = entry.read();
		expect(city.date).to.eql(new Date('2000-01-11T12:00:00Z'));
	...
	it('parses a large tile', function() {
		...
		let entry = dbpf.find({ type: FileType.cSC4City })!;
		let city = entry.read();
		expect(city.date).to.eql(new Date('2052-01-17T12:00:00Z'));
