...
class FileIndex {
	...
	async build(opts = {}) {
		...
		// respective arrays.
		this.itg = this.records.filter(Boolean).sort(itg);
	...
	// Finds all entries with the given Type and Instance ID. TODO: We'll need
	// to use a compound index for this. That's for later on though. Brute 
	// force.
	findAllTI(type, instance) {
		return this.records.filter(record => {
			return record.type === type && record.instance === instance;
		});
	}
...
// ## itg(a, b)
// TIG comparator that will sort by instance, then type and then group.
function itg(a, b) {
