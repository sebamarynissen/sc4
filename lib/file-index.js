...
class FileIndex {
	...
	async addToIndex(file) {
		...
		for (let entry of dbpf.entries) {
			...
			// If the entry is an exemplar, we'll parse it right away to check 
			// if it contains a building or prop family.
			if (entry.type === FileType.Exemplar) {
				this.exemplars.push(entry);
				let exemplar = entry.read();
				let family = exemplar.prop(Family);
				if (family) {
					let [IID] = family.value;
