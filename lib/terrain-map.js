...
class TerrainMap extends Array {
	...
	clone() {
		let clone = new TerrainMap(this.xSize, this.zSize);
	...
	fill() {
		...
		for (let z = 0; z < this.zSize; z++) {
			this[z] = new Float32Array(
				raw.buffer,
				Float32Array.BYTES_PER_ELEMENT*z*this.xSize,
				this.xSize,
			);
		}
	}
	...
	query(x, z, cliff = 0.5) {
		...
		if (!this.isCliff(i, j, cliff)) {
			...
			let R = xx > zz ? [1, 0, this[j][i+1]] : [0, 1, this[j+1][i]];
			return ipol(P, Q, R, [xx, zz]);
		} else {
			let P = [1, 0, this[j][i+1]];
			let Q = [0, 1, this[j+1][i]];
			let R = xx > 1-zz ? [1, 1, [this[j+1][i+1]]] : [0, 0, this[j][i]];
			return ipol(P, Q, R, [xx, zz]);
		}
