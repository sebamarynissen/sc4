...
import FileScanner from 'sc4/index/file-scanner.js';
...
export default async function plopAllLots(opts = {}) {
	...
	let { lots: pattern = '**/*', directory = process.cwd, logger } = opts;
	...
	let spinner = ora('Building plugin index...').start();
	const { installation, plugins } = opts;
	const index = new FileIndex({ installation, plugins });
	await index.build();
	spinner.text = 'Indexing building & prop families...';
	await index.buildFamilies();
	spinner.succeed('Plugin index built');
	...
	// Now loop all files
	spinner = ora('Plopped 0 lots').start();
	let i = 0;
	const { bbox } = opts;
