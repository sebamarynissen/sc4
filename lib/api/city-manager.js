// # city-manager.js
import { path, fs, os, hex } from 'sc4/utils';
import {
	Savegame,
	Lot,
	Building,
...
export default class CityManager {
	...
	// ## clear()
	// Clears the entire city from lots, props textures and flora.
	clear() {
		const city = this.dbpf;
		...
		for (let type of types) {
			let grids = city.readByType(FileType[type]);
			for (let grid of grids) {
				dataIds.push(grid.dataId);
			}
		}
		...
		let grid = city.getSimGrid(SimGrid.ZoneData);
		grid.clear();
