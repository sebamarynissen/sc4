...
class DependencyTrackingContext {
	...
	async track() {
		...
		let results = await Promise.all(Array.from(this.had.values()));
	...
	async readLotObjectIID(iid, lotObject, lotEntry) {
		let tasks = [];
		let entries = this.index.family(iid);
		if (!entries) {
			entries = this.index.findAll({ instance: iid });
		}

		// IMPORTANT! If we're reading the building of the lot, then our 
		// `findAll` might return stuff like LTEXT and PNG icons as well. Hence 
		// we need to make sure to only look for the building exemplar!
		if (lotObject.type === LotObjectType.Building) {
			entries = entries.filter(entry => entry.type === FileType.Exemplar);
			let exemplars = await Promise.all(
				entries.map(entry => entry.readAsync()),
			);
			let index = exemplars.findIndex(exemplar => {
				let [type] = [exemplar.value(0x10)].flat();
				return type === 0x02;
			});
			let building = entries.at(index);
			if (building) {
				entries = [building];
			}
		}
	...
	async readRktExemplar(exemplar, entry) {
		...
		for (let prop of Object.keys(props)) {
			...
			if (Array.isArray(value)) {
				if (value.length === 1) {
					let [instance] = value;
					entry = this.index.find(query = { ...hint, instance });
				} else if (value.length === 3) {
					let [type, group, instance] = value;
					entry = this.index.find(query = { ...hint, type, group, instance });
				}
			} else if (typeof value === 'number') {
				entry = this.index.find(query = { ...hint, instance: value });
			} else {
