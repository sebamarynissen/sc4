...
export default class TerrainMap extends Array {

	static [Symbol.for('sc4.type')] = FileType.TerrainMap;
	...
	flatten(i, j, h = Math.min(...this.contour(i, j))) {
	...
	egalizeX(i, j) {
		for (let di = 0; di < 2; di++) {
			let ii = i+di;
			let h = Math.min(this[j][ii], this[j+1][ii]);
			this[j][ii] = h;
			this[j+1][ii] = h;
		}
		return this;
	...
	egalizeZ(i, j) {
		for (let dj = 0; dj < 2; dj++) {
			let jj = j+dj;
			let h = Math.min(this[jj][i], this[jj][i+1]);
			this[jj][i] = h;
			this[jj][i+1] = h;
		}
		return this;
