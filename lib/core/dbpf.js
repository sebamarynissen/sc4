...
import { TGIIndex, hex, tgi } from 'sc4/utils';
...
export default class DBPF {
	...
	parse() {
		...
		this.entries = new TGIIndex(header.indexCount);

		// Header is parsed which means we now know the offset of the index. 
		// Let's parse the index as well then.
		let buffer = this.readBytes(header.indexOffset, header.indexSize);
	...
	async parseAsync() {
		let header = this.header = new Header();
		header.parse(new Stream(await this.readBytesAsync(0, 96)));
		this.entries = new TGIIndex(header.indexCount);
		let buffer = await this.readBytes(header.indexOffset, header.indexSize);
		this.index.parse(new Stream(buffer));
	}
...
class Index {
	...
	parse(rs, opts = {}) {
		...
		// Now that all entries have been added, build up the index.
		entries.build();
		...
		for (let dir of dirs) {
			...
			for (let { type, group, instance, size } of parsed) {
				let entry = dbpf.find(type, group, instance);
				if (!entry) continue;
...
function parseDir(dir, opts = {}) {
	...
	for (let i = 0; i < n; i++) {
		...
		out.push({ type, group, instance, size });
	}
	return out;
