...
// IMPORTANT! We can't use the find() function because dbpfs might contain 
// duplicate entries. We *assume* that the order of the DIR entry matches the 
// index order, but we're not entirely sure, lol.
function handleDir(dbpf, dir) {
	...
	for (let entry of entries) {
		if (
			pivot.instance === entry.instance &&
			pivot.group === entry.group &&
			pivot.type === entry.type
		) {
			entry.compressed = true;
			entry.fileSize = pivot.size;
			if (++ptr >= dir.length) break;
			pivot = dir[ptr];
		}
	}
}
