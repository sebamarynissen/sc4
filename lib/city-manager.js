...
class CityManager {
	...
	build(opts) {
		...
		// It's important to be able to know building properties to pass to 
		// the lot, such as the wealth etc. Therefore, read in the building.
		let file = building.read();
		let zoneWealth = this.getPropertyValue(file, Wealth) || 0x00;
	...
	createLot(opts) {
		...
		let lot = new Lot({
			...
			depth,
			orientation,
			zoneWealth: opts.zoneWealth || 0x00,
			zoneType: zoneTypes[0] || 0x0f,
	...
	createBuilding(opts) {
		...
		// First of all we need to find the *oriented* position of the 
		// building on the lot. This means that we need to take into account 
		// that the lot may have been rotated.
		let [xx, zz] = orient([x, z], lot);
	...
	createProp({ lot, lotObject }) {
		...
		let { OID, IIDs, orientation, x, y, z } = lotObject;
		let IID = rand(IIDs);
