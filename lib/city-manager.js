...
class CityManager {
	...
	plop(opts = {}) {
		...
		let lotExemplar = exemplars.find(record => {
			...
			let type = this.getPropertyValue(exemplar, ExemplarType);
			return type === LotConfigurations;
		});
	...
	createTexture(opts) {
		...
		// such as the tracts etc.
		let { lot, textures } = opts;
		let texture = new BaseTexture({
			mem: this.mem(),
			xMinTract: lot.xMinTract,
			zMinTract: lot.zMinTract,
			xMaxTract: lot.xMaxTract,
			zMaxTract: lot.zMaxTract,
