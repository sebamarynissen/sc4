...
function position(rect, lot) {
	...
	function move({ minX, maxX, minZ, maxZ }) {
		return {
			minX: 16*lot.minX + minX,
			maxX: 16*lot.minX + maxX,
			minZ: 16*lot.minZ + minZ,
			maxZ: 16*lot.minZ + maxZ,
		};
	}

	// First of all, if the lot has default rotation, just return the 
	// rectangle as is.
	if (lot.orientation === 0x00) {
		return move({ minX, maxX, minZ, maxZ });
	}

	// In all other cases we need the width an height of the lot **in it's 
	// local coordinates**. Note: the lot's width & depth properties are given 
	// in *local coordinates*. This means that if the orientation is east or 
	// west, we should still swap width & depth!
	let { width, depth } = lot;
	...
	// Now handle the different orientations.
