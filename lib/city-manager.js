...
class CityManager {
	...
	// account that the exemplar may possibly inherit from a parent cohort. 
	// Perhaps that we should put this in the index though...
	getProperty(file, key) {
		let prop = file.prop(key);
		if (!prop) {
			let { parent } = file;
			let entry = this.index.find(parent);
			if (entry) {
				let file = entry.read();
				prop = file.prop(key);
			}
	...
	getPropertyValue(file, key) {
		let prop = this.getProperty(file, key);
	...
	grow(opts) {
		...
		let buildingExemplar;
		if (family) {
			let buildings = family.filter(entry => {
				let file = entry.read();
				return file.value(0x10) === 0x02;
			});
			buildingExemplar = buildings[ Math.round()*buildings.length | 0];
		} else {
			let exemplars = this.index.findAllTI(FileType.Exemplar, IID)
				.filter(entry => {
					let file = entry.read();
			...
			buildingExemplar = exemplars[ exemplars.length-1 ];
		}

		// Same logic now as for plopping buildings.
		let { orientation = 0 } = opts;
		let lot = this.createLot({
			exemplar: record,
			x: opts.x,
			z: opts.z,
			orientation,
			building: buildingExemplar.instance,
		});

		// Loop all objects on the lot such and insert them.
		let { lotObjects } = props;
		for (let lotObject of lotObjects) {
			switch (lotObject.type) {
				case 0x00:
					this.createBuilding({
	...
	createLot(opts) {
		...
		let [width, depth] = file.prop(LotConfigPropertySize).value;
