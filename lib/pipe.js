...
class Pipe extends Type(FileType.PipeFile) {
	...
	parse(rs) {
		...
		this.IID = rs.dword();
		this.unknown.byte(rs.byte());
		this.firstMatrix = new Array(9).fill().map(() => rs.float());
		this.xMax = rs.float();
		this.yMax = rs.float();
		this.zMax = rs.float();
		this.xMin = rs.float();
		...
		this.unknown.float(rs.float());
		this.color1 = rs.color();
		this.x = rs.float();
		this.y = rs.float();
		this.z = rs.float();
		this.unknown.float(rs.float());
		this.unknown.float(rs.float());
		this.color2 = rs.color();
		this.x2 = rs.float();
		this.y2 = rs.float();
		this.z2 = rs.float();
		this.unknown.float(rs.float());
		this.unknown.float(rs.float());
		this.color3 = rs.color();
		this.x3 = rs.float();
		this.y3 = rs.float();
		this.z3 = rs.float();
		this.unknown.float(rs.float());
		this.unknown.float(rs.float());
		this.color4 = rs.color();
	...
	toBuffer() {
		...
		ws.dword(this.IID);
		ws.byte(unknown());
		this.firstMatrix.forEach(x => ws.float(x));
		ws.float(this.xMax);
		ws.float(this.yMax);
		ws.float(this.zMax);
		ws.float(this.xMin);
		...
		ws.float(unknown());
		ws.float(unknown());
		ws.color(this.color1);
		ws.float(this.x);
		ws.float(this.y);
		ws.float(this.z);
		ws.float(unknown());
		ws.float(unknown());
		ws.color(this.color2);
		ws.float(this.x2);
		ws.float(this.y2);
		ws.float(this.z2);
		ws.float(unknown());
		ws.float(unknown());
		ws.color(this.color3);
		ws.float(this.x3);
		ws.float(this.y3);
		ws.float(this.z3);
		ws.float(unknown());
		ws.float(unknown());
		ws.color(this.color4);
