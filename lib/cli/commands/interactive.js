...
async function submenu() {
	...
	if (!thisDir) {
		let plugins = path.resolve(process.env.HOMEPATH, 'Documents/SimCity 4/Pluginsz');
		dir = await prompts.fileSelector({
			message: 'Select the directory where the files are located',
			basePath: (fs.existsSync(plugins) ? plugins : process.cwd()),
			type: 'file+directory',
			filter(info) {
	...
	await submenuCommand(menu, '*.{dat,sc4*}', { output });
}
