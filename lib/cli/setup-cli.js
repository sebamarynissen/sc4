...
export function factory(program) {
	...
	// Command for updating the binary. Note that this is only relevant when 
	// running as sea.
	if (sea.isSea()) {
		program
			.command('update')
			.description('Update sc4 to the latest version')
			.option('-f, --force', 'Force an update without checking if the version is up to date')
