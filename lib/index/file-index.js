...
// Folder constants that we use as default. This is only used for testing 
// purposes actually when we quickly need to construct a file index. When using 
// this in actual commands, we should look it up from the config and pass it in!
const DEFAULT_PLUGINS = path.join(os.homedir(), 'Documents/SimCity 4/Plugins');
const DEFAULT_INSTALLATION = 'C:\\GOG Games\\SimCity 4 Deluxe Edition';
...
export default class FileIndex {
	...
	async getFilesToScan(opts = {}) {
		...
		if (this.options.core) {
			...
			let task = new FileScanner(installation)
				.walk()
				.then(files => coreFiles = files);
			tasks.push(task);
		}
		let task = new FileScanner(this.options.scan, { cwd: opts.cwd })
			.walk()
			.then(files => sourceFiles = files);
		tasks.push(task);
		await Promise.all(tasks);

		// Now concatenate and then sort all files.
		let files = [...coreFiles, ...sourceFiles];
		files.sort((a, b) => {
	...
	async build(opts = {}) {
		...
		const { cwd = this.options.plugins } = opts;
		const files = await this.getFilesToScan({ cwd });
