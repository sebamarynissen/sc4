...
const INSTALL_LOCATION = 'C:\\GOG Games\\SimCity 4 Deluxe Edition';
...
export default class FileIndex {
	...
	async getFilesToScan(opts = {}) {
		...
		if (this.options.core) {
			let task = this.getInstallationLocation(opts)
				.then(where => new FileScanner(where).walk())
	...
	// ## getInstallationLocation(opts)
	// Looks up the folder where SimCity 4 is installed, meaning where we can 
	// find the SimCity_*.dat files. Those are crucial for stuff like dependency 
	// tracking etc.
	async getInstallationLocation(opts = {}) {
