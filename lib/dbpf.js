...
const DBPF = module.exports = class DBPF {
	...
	constructor(file = null, opts = {}) {
		...
		// If a buffer or file was specified, parse the dbpf from it. Note: 
		// we've specified that it's *way* faster to read the file in at once. 
		// The problem is that we can't load an entire plugins directory into 
		// memory and keep it there. So if we don't want to load the file 
		// right away, it should be *explicitly* disabled, which is what we 
		// should do in that case...
		if (!opts.async) {
			if (this.file || this.buffer) {
				if (this.file && !opts.lazy) {
			...
			throw new Error(
				'Async reading of DBPF files is not supported yet!',
			);
		}
	...
	readSync() {
		...
		// entries from it.
		this.parseHeader(this.readBytesSync(0, 96));

		// Header is parsed which means we now know the offset of the index. 
		// Let's parse the index as well then.
		let buffer = this.readBytesSync(this.indexOffset, this.indexSize);
...
class Entry {
	...
	readRaw() {
		...
		this.raw = this.dbpf.readBytesSync(this.offset, this.compressedSize);
...
function parseDir(dir, opts = {}) {
	...
	let buffer = dbpf.readBytesSync(offset, size);
	let rs = new Stream(buffer);
