...
function jsCompress(input) {
	...
	// The `rev_last` variable is a lot more difficult to understand though. In 
	// the C++ code it looks like it's an array containing 256 *pointers* with 
	// an offset of 256. To port this to JS, we simulate an underlying memory 
	// block as array of integers, and have the `rev_last` array act as an 
	// array of *pointers*.
	let block = new Int32Array(256*256).fill(-1);
	let rev_last = new Uint32Array(256);
	for (let i = 1; i < 256; i++) {
	...
	for (; inpos < input.length; inpos++) {
		...
		let x = rev_last[input[inpos]] + input[inpos+1];
		let offs = rev_similar[inpos & WINDOW_MASK] = block[x];
		block[x] = inpos;
		...
		while (offs >= 0 && inpos-offs < WINDOW_LEN && i++ < QFS_MAXITER) {
			...
			let incmp = inpos + 2;
			let inref = offs + 2;
			while (input[incmp++] === input[inref++] && length < 1028) {
				length++;
			}
			if (length > bestlen) {
				bestlen = length;
...
for (let i = 0; i < 1000; i++) {
	...
	// let buffer = Buffer.from('Hello world, how are you today?');
	let js = jsCompress(buffer);
	let rev = jsDecompress(js);
	let check = Buffer.compare(rev, buffer) === 0;
	if (!check) {
		throw new Error('Whoops, not ready yet');
	}
	// let cpp = compress(buffer);
	// let check = Buffer.compare(js, cpp) === 0;
	// if (!check) {
	// 	console.log(buffer);
	// }
}

// let buffer = Buffer.from('aaaabbcc');
// // let buffer = Buffer.allocUnsafe(1024);
// console.log('-'.repeat(10), 'Now using JS', '-'.repeat(10)+'\n');
// let js = jsCompress(buffer);
// console.log(js);

// console.log('\n');
// console.log('-'.repeat(10), 'Now using C++', '-'.repeat(10));
// let cpp = compress(buffer);
// console.log('');
// console.log(cpp);
