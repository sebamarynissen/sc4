...
export default class TGIIndex extends Array {
	...
	build() {
		...
		for (let key of Object.keys(tree)) {
			...
			for (let key of Object.keys(child)) {
				child[key] = Object.groupBy(child[key], entry => h(entry.group));
			}
			child.all = byType;
		}
		this.index = tree;
	...
	findAll(query, g, i) {
		...
		// manually.
		let tgi = normalize(query, g, i);
		if (this.index === null) {
			return super.filter(createFilter(tgi));
		}
		...
		if (type !== u && group !== u && instance !== u) {
			return this.index[h(type)]?.[h(instance)]?.[h(group)] ?? [];
		}
		...
		return this.values.filter(createFilter(tgi));
