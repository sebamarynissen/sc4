...
export default class TGIIndex extends Array {
	...
	findAll(query, group, instance) {
		...
		if (known(t)) {
			if (known(i)) {
				if (known(g)) {
					let key = hash[3](t, g, i);
					let ptrs = this.index.tgi[key] ?? [];
					return ptrs.map(ptr => this[ptr]);
				} else {
					let key = hash[2](t, i);
					let ptrs = this.index.ti[key] ?? [];
					return ptrs.map(ptr => this[ptr]);
				}
			}
		} else if (known(i)) {
			let key = hash[1](i);
			let ptrs = this.index.i[key] ?? [];
			return ptrs.map(ptr => this[ptr]);
		}
...
class Index {
	...
	constructor(entries = []) {
		for (let j = 0; j < entries.length; j++) {
			...
			(this.tgi[hash[3](t, g, i)] ??= []).push(j);
			(this.ti[hash[2](t, i)] ??= []).push(j);
			(this.i[hash[1](i)] ??= []).push(j);
		}
	}
...
// The various hash functions that we use.
const hash = [
	() => undefined,
	(t) => h(t),
	(t, g) => `${h(t)}-${h(g)}`,
