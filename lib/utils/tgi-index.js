...
export default class TGIIndex extends Array {
	...
	remove(query, g, i) {
		...
		if (removed.length > 0 && this.index) {
			for (let { type, group, instance } of removed) {
				...
				let fn = createFilter({ type, group, instance });
				filterInPlace(this.index[h(type)], fn);
			}
		}
	...
	push(...tgis) {
		super.push(...tgis);
		if (this.index) {
			for (let record of tgis) {
				let { type, group, instance } = record;
				let t = h(type);
				let g = h(group);
				let i = h(instance);
				let arr = (((this.index[t] ??= {})[i] ??= {})[g] ??= []);
				arr.push(record);

				// Add the the hash-map index as well.
				(this.index[tgi(t, g, i)] ??= []).push(record);

				// Add to the type group.
				(this.index[t].all ??= []).push(record);
